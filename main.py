...
@app.route(
    "/api/v1/<string:account_type>/<int:client_id>/transaction", methods=["POST"]
)
...
def create_acc(account_type):
    ...
    if account["client_id"] in accounts.keys():
        response = make_response(
            {"status": "error", "message": "Client already has an account"}
        )
        response.status = 406
        return response
    ...
    response = make_response(
        {"status": "ok", "message": f"Account for {client_id} created"}
    )
    response.status = 201
